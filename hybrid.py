...
class Zl_hybrid_Example(object):
    ...
    for entry in input_data:
        for paragraph in entry["paragraphs"]:
            ...
            for qa in paragraph["qas"]:
                ...
                # Namvq edit squad example
                example = Zl_hybrid_Example(
                    qas_id=qas_id,
...
def create_model(bert_config, is_training, input_ids, input_mask, segment_ids,
                 zl_labels, num_zl_labels=2,
                 use_one_hot_embeddings=False):
    """
    zl_labels list of M : 0 -> is_impossible = 0 ; 1 -> is_impossible = 1
    """
    ...
    with tf.variable_scope("loss"):
        ...
        zl_one_hot_labels = tf.one_hot(
            zl_labels, depth=num_zl_labels, dtype=tf.float32)

        zl_per_example_loss = - \
            tf.reduce_sum(zl_one_hot_labels * zl_log_probs, axis=-1)
        zl_loss = tf.reduce_mean(zl_per_example_loss)

    return (start_logits, end_logits, zl_loss, zl_per_example_loss, zl_logits, zl_probabilities)


def model_fn_builder(bert_config, zl_num_labels, init_checkpoint, learning_rate,
    ...
    def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
        ...
        # Namvq edit
        is_impossible_labels = features["is_impossible"]         # [M, ]
        is_zl_labels = tf.cast(features["is_zl"], tf.float32)    # [M, ]
        ...
        # TODO Namvq call create model
        (start_logits, end_logits, zl_loss, zl_per_example_loss, zl_logits, zl_probabilities) = create_model(
            bert_config=bert_config,
            is_training=is_training,
            input_ids=input_ids,
            input_mask=input_mask,
            segment_ids=segment_ids,
            zl_labels=is_impossible_labels,
            num_zl_labels=zl_num_labels,
            use_one_hot_embeddings=use_one_hot_embeddings)
        ...
        if mode == tf.estimator.ModeKeys.TRAIN:
            ...
            def compute_loss(logits, positions, is_zl_labels):
            ...
            start_loss = compute_loss(start_logits, start_positions, is_zl_labels)
            ...
            total_loss = alpha * squad_loss + (1.0 - alpha) * classify_loss
